...
export const GalleryPageTemplate: React.FC<Props> = ({ media, config }) => {
  ...
  return (
    <>
      <Head>
        <title>{config.pageTitle} - Dune Weaver</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>

      <Header />

      <main className="min-h-screen pt-16 bg-theme-background">
        <div className="container mx-auto px-4 py-16">
          {/* Breadcrumb */}
          <div className="mb-8">
            <Link
              href="/gallery"
              className="text-theme-secondary hover:text-amber-700 flex items-center"
            >
              <svg
          ...
          <div className="text-center mb-12">
            <div className="flex items-center justify-center gap-4 mb-4">
              <h1 className="text-4xl md:text-5xl font-bold text-theme-text-primary">
                {config.heading}
              </h1>
              <span className={`px-3 py-1 rounded-full text-sm font-semibold ${config.badge.bgColor} ${config.badge.textColor}`}>
                {config.badge.text}
              </span>
            </div>
